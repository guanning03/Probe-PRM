...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if mixed_precision_config is not None:
            param_dtype = PrecisionType.to_dtype(mixed_precision_config.get("param_dtype", "fp32"))
            reduce_dtype = PrecisionType.to_dtype(mixed_precision_config.get("reduce_dtype", "fp32"))
            buffer_dtype = PrecisionType.to_dtype(mixed_precision_config.get("buffer_dtype", "fp32"))
        else:
            # Read dtype from actor config, default to bfloat16 for backward compatibility
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        elif rollout_name == "vllm_multiturn":
            assert self.config.rollout.mode == "sync", f"Async mode is not supported for multi-turn setup"

            from verl.paprika.vllm_multiturn_rollout import vLLMMultiturnRollout
            from verl.workers.sharding_manager.fsdp_vllm import FSDPVLLMShardingManager
    ...
    def update_actor(self, data: DataProto):
        ...
        with simple_timer("data_to_cpu", timing_distributed):
            data = data.to("cpu")  # data will to device with each micro batch on actor.update_policy

        assert self._is_actor
        
        with simple_timer("load_model_to_gpu", timing_distributed):
            if self._is_offload_param:
                load_fsdp_model_to_gpu(self.actor_module_fsdp)
        
        with simple_timer("load_optimizer_to_gpu", timing_distributed):
            if self._is_offload_optimizer:
                load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=get_device_id())

        with self.ulysses_sharding_manager:
            with simple_timer("sharding_preprocess", timing_distributed):
            ...
            # Extract timing breakdown from update_policy
            if "_timing_forward" in metrics:
                timing_distributed["forward"] = metrics.pop("_timing_forward")
            if "_timing_backward" in metrics:
                timing_distributed["backward"] = metrics.pop("_timing_backward")
            if "_timing_optimizer_step" in metrics:
                timing_distributed["optimizer_step"] = metrics.pop("_timing_optimizer_step")
            ...
            with simple_timer("sharding_postprocess", timing_distributed):
                output = self.ulysses_sharding_manager.postprocess_data(data=output)
            
            with simple_timer("output_to_cpu", timing_distributed):
                output = output.to("cpu")

        with simple_timer("offload_model_to_cpu", timing_distributed):
            if self._is_offload_param:
                offload_fsdp_model_to_cpu(self.actor_module_fsdp)
                log_gpu_memory_usage("After offload actor model during update_actor", logger=logger)
        
        with simple_timer("offload_optimizer_to_cpu", timing_distributed):
            if self._is_offload_optimizer:
                offload_fsdp_optimizer(optimizer=self.actor_optimizer)
                log_gpu_memory_usage("After offload actor optimizer during update_actor", logger=logger)

        # Reduce timing across all ranks and add to metrics (convert to ms)
        timing_distributed = reduce_timing(timing_distributed)
        for name, value in timing_distributed.items():
            output.meta_info["metrics"][f"timing_distributed_ms/{name}"] = value * 1000

        return output

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_gradient_estimator(self, data: DataProto):
        """
        Compute gradient statistics from Adam/AdamW optimizer state.
        Returns metrics including:
        - grad_magnitude: L2 norm of bias-corrected first moment (G_t)
        - second_moment: sum of bias-corrected second moment (M_t)
        - grad_variance: sum of variance estimate (q_hat - mu_hat^2)
        - effective_lr: sum of per-parameter effective learning rate
        - SNR: Signal-to-Noise Ratio (mu^2 / variance)
        """
        import torch.optim as optim
        
        empty_result = DataProto(meta_info={"gradient_estimator": {}})
        
        # Check if the actor and optimizer are initialized
        if not (self._is_actor and self.actor_optimizer is not None):
            if self.rank == 0:
                print(f"[GradientEstimator] Not actor or no optimizer")
            return empty_result
        
        # Check if the optimizer is Adam/AdamW
        if not isinstance(self.actor_optimizer, (optim.Adam, optim.AdamW)):
            if self.rank == 0:
                print(f"[GradientEstimator] Warning: Optimizer is not Adam/AdamW, got {type(self.actor_optimizer).__name__}")
            return empty_result
        
        # If the optimizer is offloaded, load it back
        if self._is_offload_optimizer:
            load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=get_device_id())
        
        # Check if the optimizer state is initialized 
        has_state = any(
            'exp_avg' in self.actor_optimizer.state.get(param, {}) and 'exp_avg_sq' in self.actor_optimizer.state.get(param, {})
        ...
        if not has_state:
            if self.rank == 0:
                print(f"[GradientEstimator] Warning: Optimizer state not initialized yet")
            return empty_result
        
        # Compute gradient statistics
        total_G_t = 0.0       # sum of mu_hat^2 (for grad magnitude)
        total_M_t2 = 0.0      # sum of q_hat (second moment)
        total_sigma_t2 = 0.0  # sum of (q_hat - mu_hat^2) (variance)
        total_alpha_eff = 0.0 # sum of alpha / (sqrt(q_hat) + eps)
        total_numel = 0
        ...
        for group in optimizer.param_groups:
            ...
            for p in group['params']:
                ...
                hat_mu_t = m_t / bias_correction1  # bias-corrected first moment
                hat_q_t = v_t / bias_correction2   # bias-corrected second moment
                
                # G_t: sqrt(sum_i hat_mu_i^2) - grad magnitude
                mu_sq = hat_mu_t.pow(2)
                total_G_t += mu_sq.sum().item()

                # M_t^(2): sum of hat_q
                total_M_t2 += hat_q_t.sum().item()

                # sigma^2: sum(hat_q - hat_mu^2) - variance (clamped to avoid numerical issues)
                variance_per_param = (hat_q_t - mu_sq).clamp(min=0.0)
                total_sigma_t2 += variance_per_param.sum().item()
                
                # alpha_eff: sum(alpha / (sqrt(hat_q) + eps))
                total_alpha_eff += (alpha / (hat_q_t.sqrt() + eps)).sum().item()

                total_numel += hat_mu_t.numel()

        # Compute final metrics
        diagnostics = {
            "second_moment": total_M_t2 if total_numel > 0 else 0.0,
            "grad_variance": total_sigma_t2 if total_numel > 0 else 0.0,
            "effective_lr": total_alpha_eff if total_numel > 0 else 0.0, 
            "grad_magnitude": (total_G_t ** 0.5) if total_numel > 0 else 0.0,
            # SNR = mu^2 / variance (Signal-to-Noise Ratio)
            "SNR": (total_G_t / total_sigma_t2) if (total_numel > 0 and total_sigma_t2 > 0) else 0.0,
        }
        
        return DataProto(meta_info={"gradient_estimator": diagnostics})
...
class RewardModelWorker(Worker, WorkerProfilerExtension):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.float32):
